# [딥러닝] 시각장애인을 위한 쓰레기 분리수거 이미지 분류 모델 개발

## 프로젝트 소개
쓰레기 분리수거 시, 쓰레기의 종류에 따라 어떻게 분류해야할 지 모르는 경우가 종종 발생한다.
특히, 시각장애인은 누군가의 도움이 있지 않는 이상 손으로 만져서 쓰레기를 분류하기란 쉽지 않다. 이러한 불편함을 해소하기 위해 **딥러닝을 활용해 쓰레기 분리수거 모델 개발** 하고 출력된 메세지를 오디오로 알려주는 프로젝트를 기획하게 됨
<br>

* 멤버: 본인(1명)
* 역할: 전체 기획 및 이미지 딥러닝 모델 구축 및 TTS
* 개발 기간: 23.03.26-27
<br>
...
4. 출력된 메세지를 오디오로 변환 -> API 적용(시간 관계상 개발하지 못함)
5. 추후, 휴대폰 카메라로 쓰레기를 촬영하여 이미지 판별 후 분리수거 카테고리를 안내하는 메세지 출력 및 음성을 통해 분리수거할 쓰레기를 검색하는 기능
<br>
